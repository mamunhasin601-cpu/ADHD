...
export default function RegisterScreen() {
  ...
  const error = (err: unknown, currentPhase: Phase) => {
    if (currentPhase === 'authentication') {
      Alert.alert('Аккаунт создан', authenticationAfterRegistrationMessage());
    } else if (currentPhase === 'registration') {
      const hasServerResponse = Boolean((err as { response?: unknown })?.response);
      Alert.alert(hasServerResponse ? 'Не удалось зарегистрироваться' : 'Не удалось подтвердить регистрацию', hasServerResponse ? registrationErrorMessage(err) : registrationNetworkMessage());
    } else {
      Alert.alert('Не удалось проверить контакт', contactVerificationErrorMessage(err));
    }
  };
