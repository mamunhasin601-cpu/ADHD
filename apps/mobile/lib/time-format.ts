...
export function getDeviceHourCycle(locale?: string): DeviceHourCycle {
  const resolved = new Intl.DateTimeFormat(locale, {
    ...
    hourCycle?: DeviceHourCycle;
  };
  return resolved.hourCycle ?? (resolved.hour12 ? "h12" : "h23");
}

export function uses12HourClock(
  preference: TimeFormat,
  deviceHourCycle = getDeviceHourCycle(),
): boolean {
  return (
    preference === "H12" ||
    (preference === "SYSTEM" &&
      (deviceHourCycle === "h11" || deviceHourCycle === "h12"))
  );
}

/** Presentation-only formatter. Locale and clock convention are independent. */
export function formatClockTime(
  date: Date,
  preference: TimeFormat = "SYSTEM",
...
export function formatWallClock(
  ...
  minutes: number,
  preference: TimeFormat = "SYSTEM",
  options: Omit<ClockFormatOptions, "timeZone"> = {},
): string {
