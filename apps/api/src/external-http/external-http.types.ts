...
export type ExternalFailureClass = 'timeout' | 'network' | 'http' | 'invalid-response';
...
export interface ExternalSafeLog { event: string; operation: ExternalOperationId; method: string; attempts: number; failureClass?: ExternalFailureClass; status?: number; elapsedMs: number; }
