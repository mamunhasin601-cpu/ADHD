...
## Принятое применение к Today

В соответствии с [PDR-001](../12-Decisions/PDR-001-Timeline-Centered-Day-Experience.md)
Smart Planner поддерживает таймлайн как живую модель дня, а не как жесткое
обязательство.

Для расхождения плана с реальностью обязательна последовательность:

1. нейтрально назвать изменение;
2. предложить один рекомендуемый вариант и не более двух альтернатив;
3. показать preview затронутых задач, отдыха и свободного времени;
4. дождаться принятия или редактирования;
5. оставить undo.

Приоритет recovery — сделать оставшуюся часть дня снова доступной. Smart Planner
не обязан сохранять исходный объем и никогда не компенсирует пропуск скрытой
перегрузкой будущего времени.
