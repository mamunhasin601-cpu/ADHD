...
#### 1.1.1 Visual reference direction

Today — первый reference screen для будущей visual identity, а не обещание
...
- мягкое подтверждение completion (turquoise, optional animation/haptic),
  сохранённые recovery и честный `Отменить`;
- thumb-friendly quick-add с ясной зоной касания, который не перекрывает
...
- базовый pack **Orbits**;
- будущие coherent packs **Focus Sparks** и **Focusiki**;
- будущий preview/reset в Профиле;
- размер текста;
- контраст;
- анимация;
- звук.
...
Orbits остаётся бесплатным fallback; коммерческая модель visual packs (one-time,
subscription или premium tier) пока не решена.
...
11. не перегружать экран одновременными акцентами, reward, character и motion;
12. сохранять смысл при reduced motion, отключённых haptics и масштабированном
    тексте.

Пользователь должен со временем получить настройку более спокойного visual
experience. Dark theme рассматривается только как future compatibility и не
считается реализованной capability.
