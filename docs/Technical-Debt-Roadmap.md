...
| S-06 (foundation complete; enforcement pending) | Contact-verification challenge/ticket foundation exists with Russian-provider boundary; registration enforcement and mobile PIN UI remain Task 0035. Russian placement is an explicit production requirement. Expo push and Daily.co remain unresolved foreign-service exceptions; 152-FZ compliance is not complete. | `apps/api/src/auth/contact-verification*`, `apps/api/prisma/`, `docs/ADR/ADR-010-russian-production-infrastructure-and-data-residency.md` | L | High | High | S-04; Task 0035 | 8 |
