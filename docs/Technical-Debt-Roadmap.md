...
| S-04 (OAuth configuration complete; runtime evidence pending) | Core validation, fail-closed OAuth provider configuration, and bounded external transport are complete. Production deployment/runtime verification and broader observability remain unresolved. | `apps/api/src/main.ts`, `apps/api/src/app.module.ts`, `apps/api/src/config/`, `apps/api/src/external-http/`, `.env.example` | M | Medium | High | Production operations and runtime evidence remain | 6 |
...
| S-04 (OAuth config complete) | Core validation для `NODE_ENV`/PostgreSQL/Redis/JWT/port и fail-closed OAuth config завершена; production runtime и observability остаются | M | Medium | High | C-01 |
