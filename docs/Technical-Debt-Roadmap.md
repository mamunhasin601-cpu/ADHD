...
| ID | Проблема | Файл(ы) | Сложность | Риск | Влияние | Зависимости | Порядок |
|---|---|---|---|---|---|---|---:|
| S-01 | OAuth user password генерируется через предсказуемый `Math.random()`; также создаётся usable password без необходимости. | `apps/api/src/auth/oauth.service.ts`, Prisma schema | S/M | High | High | C-01 не блокирует, но общая auth regression suite полезна | 3 |
