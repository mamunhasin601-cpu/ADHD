...
| S-04 (OAuth configuration and availability UX complete; runtime evidence pending) | Core validation, fail-closed OAuth provider configuration, public availability discovery, and bounded external transport are complete. Production deployment/runtime verification and broader observability remain unresolved. | `apps/api/src/main.ts`, `apps/api/src/app.module.ts`, `apps/api/src/config/`, `apps/api/src/external-http/`, `apps/api/src/auth/oauth-provider-availability.controller.ts`, `apps/mobile/app/auth-provider-select.tsx`, `.env.example` | M | Medium | High | Production operations and runtime evidence remain | 6 |
| S-05 (complete for payload boundary; runtime evidence pending) | Expo task-reminder payload is built through an exact typed allowlist; task/user/contact content cannot enter the external request. Expo remains a foreign-service and production data-residency exception. | `apps/api/src/notifications/notifications.payload.ts`, `notifications.service.ts`, `docs/tasks/0036-honest-privacy-safe-push-payload-allowlist.md` | M | Medium | Medium/High | S-02 | 7 |
...
| S-04 (OAuth config and availability UX complete) | Core validation для `NODE_ENV`/PostgreSQL/Redis/JWT/port, fail-closed OAuth config и public availability UX завершены; production runtime и observability остаются | M | Medium | High | C-01 |
...
| S-05 (complete for payload allowlist) | Exact Expo payload allowlist and privacy regression boundary | M | Medium | High | S-02 |
